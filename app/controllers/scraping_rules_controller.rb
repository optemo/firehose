class ScrapingRulesController < ApplicationController
  ...
  def index
    @rules = ScrapingRule.order('priority').find_all_by_product_type(params[:full] ? Session.product_type_path : Session.product_type).group_by(&:local_featurename)
    @colors = {}
    ...
    if params[:coverage] || params[:full]
      ...
      ScrapingRule.scrape(products,false,[],nil,true)[:candidates].group_by{|c|c.scraping_rule.local_featurename}.each_pair do |lf, candidates| 
  ...
  def show
    ...
    if scraping_rules.length > 1
      #Check multirules
      candidates = scraping_rules.map{|sr| ScrapingRule.scrape(products,false,ScrapingRule.find(sr),nil,true)[:candidates]}.flatten
      @candidates = Candidate.multi(candidates)
    else
      #Check single rules
      @candidates = ScrapingRule.scrape(products,false,ScrapingRule.find(params[:id]),nil,true)[:candidates].sort{|a,b|(b.delinquent ? 2 : b.scraping_correction_id ? 1 : 0) <=> (a.delinquent ? 2 : a.scraping_correction_id ? 1 : 0)}
    end
