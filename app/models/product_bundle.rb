class ProductBundle < ActiveRecord::Base
  ...
  def self.get_relations
    ...
    TextSpec.where(name: "bundle").joins("INNER JOIN cat_specs ON `text_specs`.product_id = cat_specs.product_id").where(cat_specs: {name: "product_type", value: Session.product_type_leaves}).each do |bundle|
      ...
      data.map{|d|d["sku"]}.each do |sku|
        ...
        if p_copy && p_copy.product_type == Session.product_type
          ...
          product_bundles << ProductBundle.new(bundle_id: bundle.product_id, product_type: CatSpec.find_by_name_and_product_id("product_type", bundle.product_id), product_id: p_copy.id)
    ...
    #Remove old bundles
    ProductBundle.delete_all(product_type: Session.product_type_leaves)
