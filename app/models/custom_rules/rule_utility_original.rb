class RuleUtilityOriginal < Customization
       ...
       feature_types.each do |key,features|
         ...
         Maybe(features).each  do |f|
           ...
           if (f.name == "Intercept")
             ...
             feature_value = RuleUtilityOriginal.calculateFactor_sale(org_price, saleprice)
           else
             ...
             if records[f.name][product.id]
               ...
               if f.name == "displayDate"
                 feature_value = RuleUtilityOriginal.calculateFactor_displayDate(feature_value)
                  if key == default 
                    feature_value = (1/feature_value) if feature_value != 0
                    feature_value = 1 if feature_value == 0
                  end
               elsif f.name== "saleEndDate"
                 feature_value = RuleUtilityOriginal.calculateFactor_saleEndDate(feature_value)
