class Facet < ActiveRecord::Base
  ...
  def self.check_active
     ...
     Facet.where(product_type: Session.product_type, feature_type: 'Binary').each do |facet|
       products_counts = BinSpec.joins("INNER JOIN products on products.id=product_id").joins("INNER JOIN `cat_specs` ON `bin_specs`.product_id = `cat_specs`.product_id").where(bin_specs: {name: facet.name},cat_specs: {name: :product_type, value: Session.product_type}, products: {instock: 1}).count
