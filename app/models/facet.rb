class Facet < ActiveRecord::Base
  ...
  def self.check_active
     ...
     Facet.where(product_type_id: Session.product_type_id, feature_type: 'Binary').each do |facet|
       products_counts = BinSpec.joins("INNER JOIN products on products.id=product_id").where(bin_specs: {name: facet.name, product_type: Session.p_type},products: {instock: 1}).count
