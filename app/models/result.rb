class Result < ActiveRecord::Base
  ...
  def self.upkeep_pre
    ...
    if Session.p_type == "camera_bestbuy" #Only do this for cameras
      contspecs = [] # For bulk insert
      Product.current_type.instock.each do |p|
        next if ContSpec.find_by_product_type_and_product_id_and_name(Session.p_type,p.id,"opticalzoom")
        lensrange = CatSpec.find_by_product_type_and_product_id_and_name(Session.p_type,p.id,"lensrange")
        if lensrange
          lensrange = lensrange.value
          ranges = lensrange.split("-")
          v = (ranges[1].to_f/ranges[0].to_f).round(1) #Round to one decimal point
          contspecs << ContSpec.new(:product_type => Session.p_type, :name => "opticalzoom", :product_id => p.id, :value => v)
        end
      end
      # Bulk insert
  ...
  def self.upkeep_post
    ...
    featured = BinSpec.find_all_by_product_type_and_name(Session.p_type,"featured").map(&:product)
    (featured+Product.current_type.instock).each do |product|
      saleEnd = CatSpec.find_by_product_id_and_name_and_product_type(product.id,"saleEndDate",Session.p_type)
      if saleEnd && saleEnd.value && (Time.parse(saleEnd.value) - 4.hours) > Time.now
        binspec = BinSpec.find_by_product_id_and_name_and_product_type(product.id,"onsale",Session.p_type) || BinSpec.new(:name => "onsale", :product_type => Session.p_type, :product_id => product.id)
        binspec.value = true
        binspecs << binspec
      else
        #Remove sale item if it is there
        binspec = BinSpec.find_by_product_id_and_name_and_product_type(product.id,"onsale",Session.p_type)
  ...
  def self.find_bundles
    ...
    Product.current_type.each do |p|
      bundle = TextSpec.find_by_product_type_and_product_id_and_name(Session.p_type,p.id,"bundle")
      if p.instock && bundle
        data = JSON.parse(bundle.value.gsub("=>",":"))
        data.map{|d|d["sku"]}.each do |sku|
          p_copy = Product.find_by_sku(sku)
          #Filtering out accessories
          if p_copy && p_copy.product_type == Session.p_type
            # Get or create new product bundle
            product_bundle = ProductBundle.find_or_initialize_by_bundle_id_and_product_type(p.id, Session.p_type)
            ...
            [ContSpec,BinSpec,CatSpec,TextSpec].each do |s_class|
              s_class.find_all_by_product_type_and_product_id(Session.p_type,p_copy.id).each do |spec|
                copiedspec = s_class.find_by_product_id_and_product_type_and_name(p.id,Session.p_type,spec.name) || s_class.new(:product_id => p.id, :product_type => Session.p_type, :name => spec.name)
    ...
    #Remove old bundles
    ProductBundle.delete_all(["product_type = ?", Session.p_type])
