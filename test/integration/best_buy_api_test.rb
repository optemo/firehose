...
class BestBuyApiTest < ActiveSupport::TestCase
  setup do
    # set the session department to a Best Buy department for the API calls to the BestBuy API
    Session.new 'BDepartments'
  end
  
  test "Best Buy API" do
    assert BestBuyApi.category_ids(id = 28381)
  ...
  test "keyword search" do
    skus = BestBuyApi.keyword_search("Camera", max_returned_pages = 1)
