...
task :upload_amazon_rules => :environment do |t, args|
  ...
  rules << ['release_date', 'release_date = ', '(.*release_date = )(\d{4}-\d{2}-\d{2})(.*)/\2', 'ADepartments', 'Categorical']#############
  rules << ['sales_rank', 'sales_rank = ', '(.*sales_rank = )(\d+)(.*)/\2', 'ADepartments', 'Categorical']############
  rules << ['brand', '(brand|publisher) = ', '(.*(brand|publisher) = )(.*)/\3', 'Atv_amazon', 'Continuous']###########
  ...
  rules << ['running_time', 'running_time = ', '(.*running_time = )(\d+)(.*)/\2', 'Amovie_amazon', 'Categorical']###############
