...
task :update_store_sales, [:product_type, :do_all_products, :start_date, :end_date, :directory] => :environment do |t, args|
  ...
  if args.product_type =~ /^[BbFf]/
    store = args.product_type[0]
  else
    raise "Unrecognized Product Type and Store"
  end
  
  # Get all leaf nodes within product_type specified
  if args.product_type == 'B30297' # BB tablets are mixed in with their accessories, must omit those
...
# Finds all products for each day of a given month, looks up the daily sales for these products in the 
# files sent by bestbuy (.csv), stores the sum of sales in the cont_spec table (firehose_development)
def update_store_sales (product_types, do_all_products, store, start_date, end_date, directory)
  ...
  products = CatSpec.select("sku,product_id").joins("INNER JOIN `products` ON `cat_specs`.product_id = `products`.id").where(cat_specs: {name: :product_type, value: product_types})
  ...
  if do_all_products
    ...
    ContSpec.destroy_all(:product_id => ids, :name => 'bestseller_store_sales')
